...
func (fs *FileSystem) AddFile(ctx context.Context, parent *model.Folder) (*model.File, error) {
	...
	if err != nil {
		if err := fs.Trigger(ctx, "BeforeAddFileFailed"); err != nil {
			util.Log().Debug("BeforeAddFileFailed 鉤子執行失敗，%s", err)
		}
		return nil, err
	}
	...
	if err != nil {
		if err := fs.Trigger(ctx, "AfterValidateFailed"); err != nil {
			util.Log().Debug("AfterValidateFailed 鉤子執行失敗，%s", err)
		}
...
// GetPhysicalFileContent 根據文件物理路徑獲取文件流
...
// Preview 預覽文件
//   path   -   文件虛擬路徑
//   isText -   是否為文字文件，文字文件會忽略重定向，直接由
//              服務端拉取中轉給使用者，故會對檔案大小進行限制
...
// deleteGroupedFile 對分組好的文件執行刪除操作，
// 返回每個分組失敗的文件列表
...
// GroupFileByPolicy 將目標文件按照儲存策略分組
...
// GetDownloadURL 建立文件下載連結, timeout 為資料庫中儲存過期時間的欄位
...
func (fs *FileSystem) GetSource(ctx context.Context, fileID uint) (string, error) {
	...
	if err != nil {
		return "", serializer.NewError(serializer.CodeNotSet, "無法獲取外鏈", err)
	}
...
// ResetFileIfNotExist 重設目前目標文件為 path，如果目前目標為空
...
// ResetFileIfNotExist 重設目前目標文件為 id，如果目前目標為空
...
// resetPolicyToFirstFile 將目前儲存策略重設為第一個目標文件文件使用的
