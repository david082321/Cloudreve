...
FileProgress.prototype.setChunkProgess = function (chunk_size) {
    ...
    var viewProgess = $(
        '<button class="btn btn-default">查看分塊上傳進度</button>'
    );
...
FileProgress.prototype.setProgress = function (percentage, speed, chunk_size) {
    ...
    if (this.fileProgressWrapper.find(".status").text() === "取消上傳") {
        return;
    }
    this.fileProgressWrapper
        .find(".status")
        .text("已上傳: " + size + " 上傳速度： " + formatSpeed + "/s");
...
FileProgress.prototype.setComplete = function (up, info) {
    ...
    if (uploadConfig.saveType == "oss") {
        url = "oss";
        str = "<div class='success_text'>上傳成功</div>";
    } else {
        res = $.parseJSON(info);
        if (res.url) {
            url = res.url;
