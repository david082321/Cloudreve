                ...
                if (op.uptoken) {
                    ...
                    ajax.onload = function (e) {
                        if (ajax.status === 200) {
                            ...
                            file.callbackURL = res.data.callback;
                            that.callback = res.data.callback;
                            that.callback_key = res.data.key;
                            that.key_time = res.data.key_time;
                            that.ak = res.data.ak;
                ...
                var directUpload = function (up, file, func) {
                    ...
                    if (op.save_key) {
                        ...
                        multipart_params_obj = {
                            policy: that.policy,
                            key: that.path,
                            "x-cos-meta-callback": that.callback,
                            "x-cos-meta-key": that.callback_key,
                            "q-sign-algorithm": "sha1",
                            "q-key-time": that.key_time,
                            "q-ak": that.ak,
                            "q-signature": that.token,
                        };
                    }
                    var ie = that.detectIEVersion();
                    // case IE 9-
            ...
            uploader.bind(
                ...
                (function (_FileUploaded_Handler) {
                    return function (up, file, info) {
                        ...
                        ajax = that.createAjax();
                        ajax.open("GET", file.callbackURL, true);
